...
b<-read.delim("GC_contigs.txt",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = "\t",header = F,col.names=c("contig","GC")) %>% 
  separate(contig,c("contig","delete"),sep = " ", extra = "merge") %>%
  select(-c("delete"))
#maxbin
...
#taxa
j<-read.delim("rank_names.tsv",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = "\t",header =T,col.names=c("contig","domain","phylum","class","order","family","genus","species"))
#depth
k<-read.delim("MAQ_depth.abund",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = " ",header =T) %>%
  select(-c(2)) %>%
  `colnames<-`(c("contig","MAQ_000_10_MG","MAQ_000_20_MG","MAQ_050_10_MG","MAQ_050_20_MG","MAQ_100_10_MG","MAQ_100_20_MG"))
k<-semi_join(k,b)
#kmer 4
o<-read.delim("kmer4.pca.tsv",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = "\t",header =T,col.names=c("contig","PC1_k4","PC2_k4","PC3_k4","PC4_k4","PC5_k4"))
p<-read.delim("kmer4.tsne.tsv",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = "\t",header =T,col.names=c("contig","tSNE1_k4","tSNE2_k4"))
q<-read.delim("kmer4.umap.tsv",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = "\t",header =T,col.names=c("contig","UMAP1_k4","UMAP2_k4"))
o<-o%>%
  full_join(p)%>%
  full_join(q)
#kmer 5
r<-read.delim("kmer5.pca.tsv",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = "\t",header =T,col.names=c("contig","PC1_k5","PC2_k5","PC3_k5","PC4_k5","PC5_k5"))
s<-read.delim("kmer5.tsne.tsv",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = "\t",header =T,col.names=c("contig","tSNE1_k5","tSNE2_k5"))
t<-read.delim("kmer5.umap.tsv",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = "\t",header =T,col.names=c("contig","UMAP1_k5","UMAP2_k5"))
r<-r %>%
  full_join(s) %>%
  full_join(t)
#kmer 6
u<-read.delim("kmer6.pca.tsv",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = "\t",header =T,col.names=c("contig","PC1_k6","PC2_k6","PC3_k6","PC4_k6","PC5_k6"))
v<-read.delim("kmer6.tsne.tsv",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = "\t",header =T,col.names=c("contig","tSNE1_k6","tSNE2_k6"))
w<-read.delim("kmer6.umap.tsv",quote = "",row.names = NULL,stringsAsFactors = FALSE,sep = "\t",header =T,col.names=c("contig","UMAP1_k6","UMAP2_k6"))
u<-u %>%
  full_join(v) %>%
  full_join(w)
#size
